...
class User{
    ...
    function create(){
		...
		$stmt->bindParam(':access_code', $password_hash);
		$stmt->bindParam(':status', $this->status);
	...
	public function update(){
		...
		if(empty($this->password)){
			...
			$this->address=htmlspecialchars(strip_tags($this->address));
			$this->hsz=htmlspecialchars(strip_tags($this->hsz));
			$this->emelet=htmlspecialchars(strip_tags($this->emelet));
			$this->ajto=htmlspecialchars(strip_tags($this->ajto));
			$this->csengo=htmlspecialchars(strip_tags($this->csengo));
			$this->access_level=htmlspecialchars(strip_tags($this->access_level));
            $this->status=htmlspecialchars(strip_tags($this->status));
			...
			$stmt->bindParam(':address', $this->address);
			$stmt->bindParam(':hsz', $this->hsz);
			$stmt->bindParam(':emelet', $this->emelet);
			$stmt->bindParam(':ajto', $this->ajto);
			$stmt->bindParam(':csengo', $this->csengo);
			$stmt->bindParam(':access_level', $this->access_level);
            $stmt->bindParam(':status', $this->status);
	...
	function readOne(){
		...
		$this->hsz = $row['hsz'];
		$this->emelet = $row['emelet'];
		$this->ajto = $row['ajto'];
		$this->csengo = $row['csengo'];
		$this->access_level = $row['access_level'];
		$this->password = $row['password'];
        $this->status = $row['status'];
        $this->fbsignup = $row['fbsignup'];
	}
